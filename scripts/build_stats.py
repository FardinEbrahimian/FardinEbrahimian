#!/usr/bin/env python3
"""
Render the profile activity card (dark SVG) from public GitHub data.

    python scripts/build_stats.py [out_dir]        # default: dist/

Environment:
    PROFILE_USER   GitHub login to render (default: FardinEbrahimian)
...
USER = os.environ.get("PROFILE_USER", "FardinEbrahimian")
TOKEN = os.environ.get("GITHUB_TOKEN") or os.environ.get("GH_TOKEN")
OUT_DIR = sys.argv[1] if len(sys.argv) > 1 else "dist"
UA = "profile-stats-card/1.0 (+https://github.com/FardinEbrahimian/FardinEbrahimian)"
...
def calendar_html() -> list[list[Day]]:
    ...
    for tag in re.findall(r"<td[^>]*\sdata-date=[^>]*>", page):
        ...
        if not (date and cid):
            continue
        col = re.search(r"-(\d+)-(\d+)$", cid.group(1))
        if not col:
            continue
...
def render(theme: str, weeks: list[list[Day]], stats: dict) -> str:
    ...
    for i, (w, total) in enumerate(zip(weeks, totals)):
        ...
        fill = (
            f'fill="{t["zero"]}" fill-opacity="{t["zero_op"]}"'
            if total == 0
            else 'fill="url(#bar)"'
        )
    ...
    for i, (value, label) in enumerate(tiles):
        ...
        if i:
            tile_svg.append(
                f'<path d="M{x:.1f} 92V150" stroke="{t["rule"]}" stroke-opacity="{t["rule_op"]}"/>'
            )
